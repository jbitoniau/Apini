...
// 0-999
DateHelper.getMilliseconds = function(date, asLocal) {
	return asLocal ? date.getMilliseconds() : date.getUTCMilliseconds();
};

// 0-59
DateHelper.getSeconds = function(date, asLocal) {
	return asLocal ? date.getSeconds() : date.getUTCSeconds();
};

// 0-59
DateHelper.getMinutes = function(date, asLocal) {
	return asLocal ? date.getMinutes() : date.getUTCMinutes();
};

// 0-23
DateHelper.getHours = function(date, asLocal) {
	return asLocal ? date.getHours() : date.getUTCHours();
};

// Day of month
// 1-31
DateHelper.getDate = function(date, asLocal) {
	return asLocal ? date.getDate() : date.getUTCDate();
};

// Day of week
// 0-6
DateHelper.getDay = function(date, asLocal) {
	return asLocal ? date.getDay() : date.getUTCDay();
};

// 0-11
DateHelper.getMonth = function(date, asLocal) {
...
DateHelper.getDayOfWeekName = function(
	dayIndex // 0-starting index
) {
...
// 13:49:05.305 20/12/2016
DateHelper.getFullTimeWithMillisecondsText = function(date, asLocal) {
	var text =
		...
		'.' +
		DateHelper.pad(DateHelper.getMilliseconds(date, asLocal), 3) +
		' ' +
...
// 13:49:05 20/12/2016
DateHelper.getFullTimeText = function(date, asLocal) {
	var text =
		DateHelper.pad(DateHelper.getHours(date, asLocal), 2) +
		':' +
		DateHelper.pad(DateHelper.getMinutes(date, asLocal), 2) +
...
DateHelper.getDayText = function(date, showPeriod, asLocal) {
	var text =
	...
	if (showPeriod) {
		if (
			date.getUTCHours() < 12 // https://en.wikipedia.org/wiki/12-hour_clock
		)
			text += ' AM';
		else text += ' PM';
	}
...
// Dec. 2016 Week #50
DateHelper.getWeekText = function(date, asLocal) {
	var text =
...
// December 2016
DateHelper.getMonthText = function(date, asLocal) {
	var text = DateHelper.getMonthName(DateHelper.getMonth(date, asLocal)) + ' ' + DateHelper.getFullYear(date, asLocal);
	return text;
};
